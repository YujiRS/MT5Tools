...
2. 同方向クロス抑制用配列を初期化（GlobalVariable から復元または初期化。詳細は「同方向クロス抑制」節を参照）
...
1. 同方向クロス抑制状態を GlobalVariable に保存または削除（詳細は「同方向クロス抑制」節を参照）
2. TSVログファイルを閉じる
3. チャート用 MA ハンドル解放
4. 4TF 用 MA ハンドルおよび ATR ハンドル解放
5. 矢印オブジェクト削除
...
チャート時間足の変更（`REASON_CHARTCHANGE`）では監視対象TF（M5/M15/H1/H4）は変わらないため、`lastCrossDir` を保持して同一シグナルの再通知を防止する。

ファイルスコープ変数はインジケーターの再初期化時に初期値にリセットされる場合があるため、`GlobalVariable`（ターミナルレベル永続変数）で状態を保持する。

- **OnDeinit**: `REASON_CHARTCHANGE` 時のみ deinit 理由と `lastCrossDir` を GlobalVariable に保存する。それ以外の理由（パラメータ変更、削除、ターミナル終了等）では GlobalVariable をクリアする
- **OnInit**: GlobalVariable から deinit 理由を読み取り、`REASON_CHARTCHANGE` であれば `lastCrossDir` を復元する。それ以外は `lastCrossDir` を初期化し GlobalVariable もクリアする
- **CheckTimeframe**: `lastCrossDir` 更新時に GlobalVariable にも同期保存する
- GlobalVariable 名は `ChartID()` を含めてチャートごとに一意とする
