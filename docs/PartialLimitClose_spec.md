...
- `position`: PositionTicket で指定（ヘッジ口座では PositionID と異なる場合がある）
- `volume`: 計算されたロット
...
チャート左下に `OBJ_LABEL` で以下の情報を常時表示:
- ポジション方向（BUY/SELL）、決済方向（TP/SL）、PositionID
- チケット番号の履歴（部分決済ごとに追加）
- 元ロット → 残ロット
- 各レベルの状態（済/待機）

表示は OnInit 時に作成、ExecutePartialClose 成功後に更新、OnDeinit で削除。
...
部分決済の進捗状態をファイルに保存し、MT5再起動後に復元する。

### 保存ファイル
- パス: `MQL5/Files/PLC_state_<PositionID>.txt`
- テキスト形式、1行1項目

### 保存する状態
- 入力パラメータのスナップショット（復元時の一致判定用）
- 元ロット（`gOrigLot`）
- 各レベルの完了フラグ
- 最終チケット番号
- チケット履歴文字列
...
### 保存タイミング
- 部分決済成功後（`ExecutePartialClose` 内）

### 復元タイミング
- `OnInit` でレベル解析後に復元を試行

### 復元条件
- 状態ファイルが存在する
- ファイル内の入力パラメータが現在の input と完全一致
- **不一致の場合**: ファイルを削除し、フレッシュスタート

### 復元時の動作
- `gOrigLot` を保存値で上書き（再起動時の残ロットではなく初回起動時の元ロットを使用）
- 完了済みレベルの `done` フラグを復元
- 完了済みレベルはチャートライン描画・価格方向チェックをスキップ
- チケット履歴を復元

### 状態ファイル削除タイミング
- 全レベル決済完了時（`ExpertRemove` 前）
- ポジション外部決済検出時（`ExpertRemove` 前）
- `OnDeinit` では削除しない（再起動に備えて保持）
