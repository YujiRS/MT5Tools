# 依頼：MT5 / MQL5 既存ポジション決済EAの作成（Ticket指定・SMAクロス）
...
## 1) 入力（input）
1) 監視TF（クロス判定に使うTF）
- input ENUM_TIMEFRAMES SignalTF = PERIOD_M5;  // 例

2) 対象ポジション Ticket
- input ulong TargetTicket = 0;  // 0は禁止（必須入力）

3) （任意）最大許容スプレッド（見送り機能）
- input bool UseSpreadGuard = false;
- input double MaxSpreadPoints = 0;  // UseSpreadGuard=trueの時のみ使用（ポイント単位）
...
- EA起動時に以下をチェック：
  1) TargetTicket のポジションが存在すること（存在しなければエラーをログ出して停止/待機）
